...
class AdvancedObstacleDetector:
    def __init__(self):
        print("Loading Advanced Obstacle Detector (High Accuracy, Low Noise)...")
        
        # ================== INITIALIZE DETECTORS ==================
        # 1. Background Subtractor (phát hiện vật mới)
        ...
        # 2. Cascade Classifier cho vật thể lớn
        self.object_cascade = None
        self._load_cascade()
        
        # ================== NOISE FILTERING ==================
        # Temporal filtering (lọc nhiễu thời gian)
        self.detection_history = deque(maxlen=5)  # Lưu 5 frame gần nhất
        self.confidence_history = deque(maxlen=3)  # Lưu 3 confidence gần nhất
        
        # Spatial filtering parameters
        self.min_obstacle_area = 1000  # pixel - vật phải đủ lớn
        self.max_obstacle_area = 50000  # pixel - vật không quá lớn
        self.min_aspect_ratio = 0.3  # Tỉ lệ chiều rộng/chiều cao tối thiểu
        self.max_aspect_ratio = 3.0  # Tỉ lệ chiều rộng/chiều cao tối đa
        
        # Confidence thresholds
        self.min_confidence = 0.65  # 65% confidence trở lên
        self.min_detections = 2    # Cần 2 lần phát hiện liên tiếp
        
        # Region of Interest (chỉ phát hiện ở vùng phía trước)
        self.roi_y_start = 0.3  # Bắt đầu từ 30% chiều cao frame
        self.roi_y_end = 0.95   # Kết thúc ở 95% chiều cao frame
        
        # State tracking
        self.last_obstacle_time = 0
        self.obstacle_cooldown = 5  # giây
        self.obstacle_count = 0
        
        # FIX: THÊM BIẾN ĐỂ THEO DÕI LIÊN TỤC
        self.obstacle_tracker = {}  # Track obstacles qua các frame
        self.track_id_counter = 0
        self.max_track_age = 10  # Số frame tối đa không thấy thì xóa track
        
        # FIX: THÊM CỬA SỔ THỜI GIAN ĐỂ GIẢM NHẢY
        self.obstacle_window = deque(maxlen=10)  # Lưu trạng thái 10 frame gần nhất
        
        print("Advanced Obstacle Detector ready")
    
    def _load_cascade(self):
        """Tải cascade classifier nếu có"""
        try:
            cascade_paths = [
            ...
            for cascade_path in cascade_paths:
                if os.path.exists(cascade_path):
                    ...
                    if not self.object_cascade.empty():
                        print(f"   Loaded object cascade: {os.path.basename(cascade_path)}")
                        return
            
            print("   No object cascade found, using motion detection only")
            self.object_cascade = None
    ...
    def _update_obstacle_tracker(self, obstacles, frame_shape):
        """Cập nhật tracker để theo dõi vật cản qua các frame"""
        h, w = frame_shape[:2]
        current_time = time.time()
        
        # Danh sách obstacles hiện tại với vị trí
        current_obstacles = []
        ...
        for track_id, track_info in list(self.obstacle_tracker.items()):
            if current_time - track_info['last_seen'] > self.max_track_age:
                to_remove.append(track_id)
        
        for track_id in to_remove:
            del self.obstacle_tracker[track_id]
        
        # Gán obstacles hiện tại với tracks cũ (matching)
        matched_tracks = set()
        ...
        # Tạo tracks mới cho obstacles chưa được match
        for idx, obs in enumerate(current_obstacles):
    ...
    def _get_stable_obstacles(self, min_age=3):
        """Lấy vật cản đã xuất hiện ổn định"""
        stable_obstacles = []
        
        for track_id, track_info in list(self.obstacle_tracker.items()):
            # Chỉ lấy obstacles đã xuất hiện ít nhất min_age frame
            if track_info['age'] >= min_age:
    ...
    def _detect_using_cascade(self, frame):
        """Sử dụng cascade classifier cho vật thể cụ thể"""
        if self.object_cascade is None or frame is None:
            return []
        
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        
        # Cải thiện ảnh trước khi detect
    ...
    def _detect_using_background_subtraction(self, frame):
        """Phát hiện vật cản bằng background subtraction (chính xác)"""
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        ...
        # Xử lý morphology để nối các vùng gần nhau
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
        fgmask = cv2.morphologyEx(fgmask, cv2.MORPH_OPEN, kernel)   # Loại bỏ noise nhỏ
        fgmask = cv2.morphologyEx(fgmask, cv2.MORPH_CLOSE, kernel)  # Nối các vùng gần
        
        # Áp dụng threshold với Otsu
        ...
        for contour in contours:
            ...
            if self.min_obstacle_area <= area <= self.max_obstacle_area:
                ...
                if self.min_aspect_ratio <= aspect_ratio <= self.max_aspect_ratio:
                    ...
                    # Lọc dựa trên solidity
                    if solidity > 0.3:
                        # Tính confidence
                        area_confidence = min(1.0, area / 8000)
                        solidity_confidence = solidity
                        confidence = 0.6 * area_confidence + 0.4 * solidity_confidence
    ...
    def _temporal_filter(self, current_obstacles):
        ...
        # FIX: Sử dụng cửa sổ thời gian để giảm nhảy
        ...
        for current_obs in current_obstacles:
            ...
            for hist_obstacles in list(self.detection_history)[-self.min_detections:]:
            ...
            if appearance_count >= self.min_detections:
                consistency_confidence = min(1.0, appearance_count / self.min_detections * 1.2)
    ...
    def detect_obstacles(self, frame):
        ...
        # FIX: Cập nhật tracker
        self._update_obstacle_tracker(final_obstacles, frame.shape)
        
        # FIX: Lấy obstacles ổn định từ tracker
        stable_obstacles = self._get_stable_obstacles(min_age=3)
        ...
        # 8. Non-maximum suppression (tránh trùng lặp)
    ...
    def visualize_detection(self, frame, obstacles, danger_level):
        ...
        for obs in obstacles:
            ...
            # FIX: Thêm track_id nếu có
            track_id = obs.get('track_id', None)
            age = obs.get('age', 1)
            
            # Kiểm tra proximity
            proximity = self._detect_obstacle_proximity((x, y, w_bbox, h_bbox), w, h)
    ...
    def _detect_obstacle_proximity(self, bbox, frame_width, frame_height):
        """Phát hiện mức độ gần của vật cản với camera"""
        x, y, w, h = bbox
        
        proximity_score = 0.0
        
        if y + h >= frame_height - 10:
            proximity_score += 0.4
        
        if x <= 10 and x + w >= frame_width - 10:
            proximity_score += 0.4
