...
def insert_face(name: str, relationship: str, email: Optional[str], image_path: str, embedding):
    ...
    try:
        ...
        if existing:
            print(f"⚠️ [FACE_DB] Face already exists: {name}")
            conn.close()
            return False
        ...
        try:
            ...
            print(f"⚠️ [FACE_DB] Cannot import face_model to sync cache: {e}")
    ...
    try:
        ...
        for r in rows:
            try:
                ...
                print(f"⚠️ [FACE_DB] Error parsing face {r['faceID']}: {e}")
                continue
...
def delete_face(face_id: int) -> bool:
    ...
    try:
        print(f"🗑️ [FACE_DB] Deleting face ID: {face_id}")
        ...
        if not row:
            print(f"⚠️ [FACE_DB] Face not found: {face_id}")
            conn.close()
            return False
                ...
                try:
                    ...
                    for img_path in possible_paths:
                        if img_path and os.path.exists(img_path):
                            os.remove(img_path)
                            print(f"🗑️ [FACE_DB] Image file deleted: {img_path}")
                            break
                except Exception as img_error:
                    print(f"⚠️ [FACE_DB] Could not delete image: {img_error}")
...
def sync_cache_after_operation():
    ...
    try:
        ...
        print(f"⚠️ [FACE_DB] Cache sync failed: {e}")
        return False
